...
async def test_factory_with_password(monkeypatch):
    monkeypatch.setattr("pydaikin.daikin_skyfi.DaikinSkyFi.__init__", lambda self, ip, session, password: None)
...
async def test_factory_with_key(monkeypatch):
    monkeypatch.setattr("pydaikin.daikin_brp072c.DaikinBRP072C.__init__", lambda self, ip, session, key, uuid, ssl_context: None)
...
async def test_factory_brp084(monkeypatch):
    class DummyBRP084:
        def __init__(self, ip, session): pass
...
async def test_factory_brp069(monkeypatch):
    ...
    monkeypatch.setattr("pydaikin.daikin_brp084.DaikinBRP084.__init__", lambda self, ip, session: (_ for _ in ()).throw(DaikinException("fail")))
    device = await DaikinFactory("192.168.1.2")
...
async def test_factory_airbase(monkeypatch):
    ...
    monkeypatch.setattr("pydaikin.daikin_brp084.DaikinBRP084.__init__", lambda self, ip, session: (_ for _ in ()).throw(DaikinException("fail")))
    monkeypatch.setattr("pydaikin.daikin_brp069.DaikinBRP069.__init__", lambda self, ip, session: (_ for _ in ()).throw(DaikinException("fail")))
